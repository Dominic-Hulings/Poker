...
PokerLog::PokerLog( Dealer* inpDealer, int inToAct, std::vector<Player*>* inpPlayersVec, int inPlayersToDeal,
                    int inLittleBlind, int inBigBlind )
{
  this->toAct = inToAct;
...
vector<CT::Card> PokerLog::TopFiveCards( vector<Card> cards, int handStrength, vector<pair<string, int>> pairs,
                                         int straightHighestCardValue, string suit )
{
  ...
  case 4:
    ...
    break;
  case 5:
    valuesInStraight = { straightHighestCardValue - 4, straightHighestCardValue - 3, straightHighestCardValue - 2,
                         straightHighestCardValue - 1, straightHighestCardValue };
    int valueSeenIndex;

    for ( Card card : cards )
    {
      if ( !( ConvertValueToNum( card.first ) >= straightHighestCardValue - 4 &&
              ConvertValueToNum( card.first ) <= straightHighestCardValue ) )
      {
      ...
      else if ( ( valueSeenIndex = find( valuesInStraight.begin(), valuesInStraight.end(),
                                         ConvertValueToNum( card.first ) ) != valuesInStraight.end() ) )
      {
...
{
  string possibleHands[10] = { "High Card", "One Pair",   "Two Pair",       "Three-of-a-Kind", "Straight",
...
void PokerLog::Action()
{
  ...
  while ( true )
  {
    ...
    cout << "Your hand is: " << playersVec.at( toAct )->GETplayerHand().first.first << " of "
         << playersVec.at( toAct )->GETplayerHand().first.second;
    cout << "\nAnd: " << playersVec.at( toAct )->GETplayerHand().second.first << " of "
         << playersVec.at( toAct )->GETplayerHand().second.second;
    cout << "\n\n";
...
Player* PokerLog::WhoWon( std::vector<Player*> players, std::vector<Card> field )
{
  ...
  for ( Player* player : players )
  {
    ...
    currentPlayerHand.second.second =
        TopFiveCards( fieldAndHand, currentPlayerHand.second.first, Pairs, straightHighestCardValue, mostSuit );
